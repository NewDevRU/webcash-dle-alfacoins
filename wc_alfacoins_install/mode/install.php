...
if ($config['skin'] != 'Default') {
	$path = ROOT_DIR.'/wc_alfacoins_install/files/templates/Default/';
	if (file_exists($path)) {
...
$fname = ROOT_DIR.'/wc_alfacoins_install/sql/install.sql';
	...
	foreach ($webcash_config as $section_key => $section_value) {
		...
		if ($was_changed) {
			if (!$only_once_flag) {
				...
				$bak_fname = str_replace('.'.$extension, '.alfacoins.bak.'.$extension, $fname);
				nd_copy($fname, $bak_fname);
				echo '<li>Создана резервная копия файла &laquo;'.str_replace(ROOT_DIR, '', $bak_fname).'&raquo;</li>';
				echo '<li>В файл &laquo;/engine/data/config.webcash.php&raquo; добавлены соответствующие изменения из файла &laquo;/wc_alfacoins_install/append/config.webcash.php&raquo;</li>';
			}
...
<p>Установка успешно завершена, папка &laquo;wc_alfacoins_install&raquo; может быть удалена. Вы можете <a href="<?php echo $webcash->module_admin_url.'&tab='.Admin_Panel::GATEWAYS_TAB.'&action=addonsett&gw_alias=alfacoins'; ?>" target="_blank">перейти в админцентр</a>, чтобы настроить установленный шлюз или <a href="/" target="_blank">перейти на Главную страницу</a> сайта.</p>
